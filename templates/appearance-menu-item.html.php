                    ...
                    <div class="col-sm-10">
                        <select name="menu_type" id="menu_type" class="custom-select">
                            <?php foreach ($this->types as $type): ?>
                                <option value="<?= $type['menu_type']; ?>"
                                    <?= $type['menu_type'] === $this->item->getType() ? ' selected' : ''; ?>>
                                    <?= $type['title']; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                        ...
                        <select name="parent_id" id="parent_id" class="custom-select">
                            ...
                            <?php foreach ($this->items->getByType($this->item->getType(), false) as $parent): ?>
                                ...
                                <option value="<?= $parent->getId(); ?>"
                                    <?= $parent->getId() === $this->item->getParentId()
                                        ? ' selected' : ''; ?>>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <select name="item_type" id="item_type" class="custom-select custom-select-sm">
                            <option value=""<?= $this->item->getItemType() !== 'absolute'
                                ? ' selected="selected"' : ''; ?>>Aplikacja
                            </option>
                            <option value="absolute"
                                <?= $this->item->getItemType() === 'absolute' ? ' selected' : ''; ?>>
                                Zewnętrzna
                            </option>
                        </select>
                    </div>
